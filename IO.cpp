...
static Uint32 mColors [COLOR_MAX] = {0x000000ff, 0x878181ff,					// Colors
                               0xff0000ff, 0x00ff00ff, 0x0000ffff,
                               0x00ffffff, 0xff00ffff, 0xffff00ff,
                               0xffffffff, 0xff0f0fff, 0xa21820ff };
...
void IO::ClearScreen() 
{
	boxColor (mScreen, 0, 0, mScreen->w - 1, mScreen->h - 1, mColors[BACKGROUND]);
	//boxRGBA(mScreen, 0, 0, mScreen->w - 1, mScreen->h - 1, 243, 177, 177, 1);
}
...
void IO::DrawClearRectangle( int pX1, int pY1, int pX2, int pY2, enum color pC)
{
	rectangleColor (mScreen, pX1, pY1, pX2, pY2 - 1, mColors[pC]);
}


void IO::DrawString(int pX, int pY, char *score, enum color pC)
{
	stringColor(mScreen, pX, pY, score, mColors[pC]);
}
